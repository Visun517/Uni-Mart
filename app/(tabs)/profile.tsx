          ...
          <View className="p-1 bg-white rounded-full shadow-2xl">
            <View className="w-32 h-32 overflow-hidden border-4 border-white rounded-full bg-slate-100">
              {profilePic || user?.profilePic ? (
                <Image
                  source={{ uri: profilePic || user?.profilePic }}
      ...
      >
        <ScrollView showsVerticalScrollIndicator={false} className="px-8">
          <View className="p-6 bg-white border shadow-xl rounded-3xl shadow-slate-400 border-slate-50">
            ...
            <View className="mb-6">
              <Text className="mb-3 ml-1 text-[10px] font-black tracking-widest text-slate-400 uppercase">
                Account Identity
              </Text>
              <View className="flex-row items-center px-4 border h-14 bg-slate-50 rounded-2xl border-slate-100">
                ...
                <TouchableOpacity
                  onPress={() =>
                    isEditing ? handleUpdateName() : setIsEditing(true)
                  }
                  className="p-2"
                >
                  {isEditing ? (
            ...
            <View>
              ...
              <View className="flex-row items-center px-4 border h-14 bg-slate-50/50 rounded-2xl border-slate-50">
                ...
                <Text className="ml-3 text-base font-semibold text-slate-400">
                  {user?.email}
                </Text>
              </View>
            </View>
          ...
          <View className="mt-8 space-y-3">
            <Text className="mb-2 ml-1 text-[10px] font-black tracking-widest text-slate-400 uppercase">
              Preferences
            </Text>
            ...
            {loading ? (
              ...
              >
                ...
                <Text className="text-lg font-black text-red-500">
                  Sign Out from Uni-Mart
                </Text>
              </TouchableOpacity>
            )}
            <Text className="mt-6 text-xs font-bold tracking-widest text-center uppercase text-slate-300">
