...
import { Trash2, ShoppingCart, Hexagon, ArrowRight, ShoppingBag } from "lucide-react-native";
...
function CartScreen() {
  ...
  // මුළු වටිනාකම ගණනය කිරීම (Total Price Calculation)
    ...
    <View className="flex-1 bg-white">
      ...
      <View className="bg-[#1A3BA0] pt-14 pb-20 px-8 rounded-b-[45px] shadow-2xl">
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center">
            ...
            <View>
              <Text className="text-sm font-bold tracking-widest uppercase text-blue-100/70">Selected Items</Text>
              <Text className="text-3xl font-black tracking-tight text-white">My Cart</Text>
            </View>
          </View>
          <View className="items-end">
             <Text className="text-2xl font-black text-white">{cartItems.length}</Text>
             <Text className="text-blue-100/70 text-[10px] font-bold uppercase">Items</Text>
          </View>
        </View>
      </View>
      ...
      <View className="px-8 -mt-10">
        <View className="flex-row items-center justify-between p-6 bg-white border shadow-xl rounded-3xl shadow-slate-400 border-slate-50">
          <View>
            <Text className="text-xs font-bold tracking-widest uppercase text-slate-400">Subtotal Amount</Text>
            <Text className="mt-1 text-3xl font-black text-[#1A3BA0]">Rs. {totalPrice.toLocaleString()}</Text>
          </View>
          <TouchableOpacity 
      ...
      <View className="flex-1 px-6 mt-8">
        <Text className="px-1 mb-4 text-xl font-black text-slate-900">Cart Inventory</Text>
        ...
        {loading && !refreshing ? (
          ...
          <FlatList
            ...
            refreshControl={
              <RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor="#1A3BA0" />
            }
            renderItem={({ item }) => (
              <TouchableOpacity 
                activeOpacity={0.8}
              ...
              >
                <Image source={{ uri: item.imageUrl }} className="w-20 h-20 rounded-[20px] bg-slate-100" />
                <View className="flex-1 ml-4">
                  <Text className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1">{item.category}</Text>
                  <Text className="text-lg font-bold text-slate-800" numberOfLines={1}>{item.title}</Text>
                  <Text className="text-base font-black text-slate-900">Rs. {item.price.toLocaleString()}</Text>
                </View>
                <TouchableOpacity 
            ...
            ListEmptyComponent={
              <View className="items-center justify-center px-10 mt-20">
                ...
                >
                    <Text className="font-black text-white">Explore Marketplace</Text>
                </TouchableOpacity>
              </View>
            }
          />
      ...
      {cartItems.length > 0 && (
          <View className="absolute bottom-10 left-8 right-8">
              ...
              >
                  ...
                  <Text className="text-lg font-black text-white">Place Orders Now</Text>
              </TouchableOpacity>
          </View>
      )}
    </View>
